...
| `/users/:id` | جزئیات | **بالا:** کارت «اطلاعات اتصال برای مشتری» (کپی / دانلود ovpn)؛ پایین: `mikrotik_name` (ثابت), ویرایش رمز/تماس/`notes`, assign, رزرو, تاریخچه — **بدون** `comment` روتر |
...
4. جدول تاریخچه activation به‌روز می‌شود (بدون مبلغ → «ثبت نشده»)
...
بقیه صفحه همان `/users/:id` مدیر: کارت اتصال مشتری، رمز/تماس/`notes`, assign, رزرو, تاریخچه — از `/admin/vpn-users/:id/connection` و `.../ovpn` در صورت نیاز.
...
│  (فرم ویرایش، تمدید، پروفایل‌ها، تاریخچه)                                │
└─────────────────────────────────────────────────────────────────────────┘
```
