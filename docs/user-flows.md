...
| `/users/new` | ایجاد | پیشوند ثابت (`name_prefix` از `/me`) + textbox `local_name` + پیش‌نمایش نام کامل روتر — **بدون** فیلد گواهی |
...
| `/admin/managers` | لیست مدیران — ایجاد با slug بدون همپوشانی، ویرایش quota، غیرفعال/فعال، **عنوان گواهی** در ایجاد و ویرایش inline |
| `/admin/settings` | تنظیمات ادمین | همان `/settings` مدیر (نام نمایشی + رمز) |
| `/admin/users` | لیست همه VPN — فیلتر پیش‌فرض **همه**؛ `?orphan=true` یا `?manager_id=` برای محدود کردن |
| `/admin/users/new` | ایجاد کاربر VPN — مدیر اختیاری + **عنوان گواهی** اختیاری (`POST /admin/vpn-users`) |
...
## فرم ایجاد کاربر / مدیر — گواهی (فقط ادمین)

```
┌─────────────────────────────────────────────┐
│ … فیلدهای معمول ایجاد …                   │
│ عنوان گواهی (اختیاری)  [______________]   │
│  → در صورت پر شدن: ساخت گواهی در روتر    │
│     + config-{نام_کاربر}.ovpn             │
└─────────────────────────────────────────────┘
```

- فقط در `/admin/users/new` و فرم **ایجاد** مدیر در `/admin/managers`.
- مدیر در `/users/new` این فیلد را **نمی‌بیند**.
- پسورد کلید خصوصی در UI نمایش داده نمی‌شود — پنل تولید و ذخیره می‌کند ([certificates.md](certificates.md)).
- چند کاربر می‌توانند **همان** `cert_title` را داشته باشند (بدون محدودیت یکتایی در DB).

## فرم ویرایش — گواهی (فقط ادمین)

### `/admin/managers` — ویرایش inline

```
┌─────────────────────────────────────────────┐
│ … نام کاربری / سقف / فعال …               │
│ عنوان گواهی  [______________]  (ستون جدول) │
│  → پر بود و عوض شد → دیالوگ تأیید (مثل کاربر) │
│  → خالی = پاک meta در DB                  │
└─────────────────────────────────────────────┘
```

### `/admin/users/:id` — همان کارت ویرایش کاربر

```
┌─────────────────────────────────────────────┐
│ … رمز VPN / اتصال همزمان / یادداشت …      │
│ عنوان گواهی (اختیاری)  [______________]   │
│  → خالی بود → عنوان جدید = صدور بدون تأیید│
│  → پر بود و عوض شد → دیالوگ تأیید        │
│  → پر بود و خالی شد = فقط پاک DB         │
└─────────────────────────────────────────────┘
```
...
| `cert_title` | **عنوان گواهی** | فقط `/admin/users/:id` — رفتار [certificates.md](certificates.md) |
...
| OpenVPN Private Key Password | اولویت: گواهی کاربر → گواهی مدیر → env — مدیر تفاوت را در UI نمی‌بیند |
...
| فایل `.ovpn` | اگر گواهی: خواندن `config-{mikrotik_name}.ovpn` از روتر؛ وگرنه قالب legacy |
