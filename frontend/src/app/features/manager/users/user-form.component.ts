...
@Component({
  ...
  template: `
    ...
    <form class="card form" (ngSubmit)="submit()">
      ...
      <label>تلفن <input [(ngModel)]="contactPhone" name="phone" /></label>
      <label>یادداشت تماس <input [(ngModel)]="contactNote" name="cnote" /></label>
      <label>توضیحات (پنل) <textarea [(ngModel)]="notes" name="notes" rows="2"></textarea></label>
      <label class="check">
...
export class UserFormComponent implements OnInit {
  ...
  submit(): void {
    ...
    const body = {
      ...
      contact_note: this.contactNote || undefined,
